...
Deno.test("getStaleFiles: brand strategy cascades to identity and model", () => {
  ...
  assertEquals(stale.includes("business/02b-brand-identity.yaml"), true);
  assertEquals(stale.includes("business/03-business-model.md"), true);
  assertEquals(stale.includes("business/04-value-proposition.md"), true);
  assertEquals(stale.includes("business/05-personas-jobs.md"), true);
});

Deno.test("getStaleFiles: brand identity cascades to website styles", () => {
  const stale = getStaleFiles(["business/02b-brand-identity.yaml"]);
  assertEquals(stale.includes("website/assets/styles.css"), true);
});

Deno.test("getStaleFiles: sitemap cascades to SEO brief", () => {
  const stale = getStaleFiles(["business/06-sitemap.yaml"]);
  assertEquals(stale.includes("business/08-seo-brief.md"), true);
});
...
Deno.test("getStaleFiles: no cascade for leaf files", () => {
  const stale = getStaleFiles(["business/10-launch-checklist.md"]);
...
Deno.test("getStaleFiles: content deck cascades to routes", () => {
  const stale = getStaleFiles(["business/09-content-deck.md"]);
...
Deno.test("getStaleFiles: multiple changes don't duplicate", () => {
  ...
  const sitemapCount = stale.filter(f => f === "business/06-sitemap.yaml").length;
